import { useState, useRef } from "react";
import { useNavigate } from "react-router-dom";
import "./newrelease.css";
import MiniPlayer from "/Users/huynh/music-app/src/components/User/MiniPlayer";
export default function NewRelease() {
   const navigate = useNavigate();
  const [tab, setTab] = useState("all");
  const [songList, setSongList] = useState([]);
  const [currentIndex, setCurrentIndex] = useState(null);
  // === Danh sách bài hát, mỗi bài cần có id ===
  const songsAll = [
   { 
    id: "song-1",
    title: "The Fate of Ophelia",
    artist: "Taylor Swift, The Chainsmokers",
    album: "Midnight Tales",
    duration: "3:42",
    img: "https://image-cdn.nct.vn/song/2025/11/28/m/Z/m/l/1764309023569_300.jpg",
    label: "UNIVERSAL MUSIC GROUP",
    labelImg: "https://image-cdn.nct.vn/provider/2020/02/06/7/c/c/b/1580970856708.jpg",
    audio: "/music/The Fate of Ophelia.mp3"
  },
  { 
    id: "song-2",
    title: "Panorama",
    artist: "Richie D. ICY, Obito",
    album: "Panorama EP",
    duration: "3:18",
    img: "https://image-cdn.nct.vn/song/2025/10/11/q/M/D/Q/1760122306996_300.jpg",
    label: "WARNER RECORDED MUSIC",
    labelImg: "https://image-cdn.nct.vn/provider/2020/11/12/e/2/a/c/1605147272711.jpg",
    audio: "/music/Panorama.mp3"
  },
  {
    id: "song-3",
    title: "画圈",
    artist: "EN",
    album: "画圈 (Single)",
    duration: "4:01",
    img: "https://image-cdn.nct.vn/song/2025/11/26/t/T/S/X/1764125550570_300.jpg",
    label: "UNIVERSAL MUSIC GROUP",
    labelImg: "https://image-cdn.nct.vn/provider/2020/02/06/7/c/c/b/1580970856708.jpg",
    audio: "/music/画圈.mp3"
  },
  {
    id: "song-4",
    title: "BOUNCE",
    artist: 'ANH TRAI "SAY HI"',
    album: "Say Hi Collection",
    duration: "3:27",
    img: "https://image-cdn.nct.vn/song/2025/11/24/x/s/k/Q/1763957213536_300.jpg",
    label: "UNIVERSAL MUSIC GROUP",
    labelImg: "https://image-cdn.nct.vn/provider/2020/02/06/7/c/c/b/1580970856708.jpg",
    audio: "/music/BOUNCE.mp3"
  },
  {
    id: "song-5",
    title: "NOT CUTE ANYMORE",
    artist: "ILLIT",
    album: "ILLIT Debut",
    duration: "2:58",
    img: "https://image-cdn.nct.vn/song/2025/11/24/g/I/I/y/1763980894698_300.jpg",
    label: "INGROOVES MUSIC GROUP",
    labelImg: "https://image-cdn.nct.vn/provider/2023/07/06/4/f/c/4/1688614662954.jpg",
    audio: "/music/NOT CUTE ANYMORE.mp3"
  },
  {
    id: "song-6",
    title: "Havana",
    artist: "Camila Cabello, Young Thug",
    album: "Camila",
    duration: "3:37",
    img: "https://image-cdn.nct.vn/song/2017/08/07/c/b/1/e/1502075660969_300.jpg",
    label: "SONY MUSIC",
    labelImg: "https://image-cdn.nct.vn/provider/2020/02/13/8/d/9/4/1581563932392.jpg",
    audio: "/music/Havana.mp3"
  },

    {
  id: "song-7",
  title: "APT.",
  artist: "ROSÉ, Bruno Mars",
  album: "APT. (Single)",
  duration: "2:49",
  img: "https://image-cdn.nct.vn/song/2024/10/18/1/8/a/e/1729211353297_300.jpg",
  audio: "/music/APT..mp3",
  label: "WARNER RECORDED MUSIC",
  labelImg: "https://image-cdn.nct.vn/provider/2020/11/12/e/2/a/c/1605147272711.jpg",
},
{
  id: "song-8",
  title: "ĐOẠN KỊCH CÂM",
  artist: 'ANH TRAI "SAY HI"',
  album: "Say Hi Collection",
  duration: "3:45",
  img: "public/topic/1.jpg",
  audio: "/music/ĐOẠN KỊCH CÂM.mp3",
  label: "UNIVERSAL MUSIC GROUP",
  labelImg: "https://image-cdn.nct.vn/provider/2020/02/06/7/c/c/b/1580970856708.jpg",
},
{
  id: "song-9",
  title: "Anh Sẽ Quên Được Em",
  artist: "QuocKiet, Hayho",
  album: "Anh Sẽ Quên Được Em (Single)",
  duration: "4:02",
  img: "https://image-cdn.nct.vn/song/2025/11/25/1/b/e/5/1764049685571_300.jpg",
  audio: "/music/Anh Sẽ Quên Được Em.mp3",
  label: "LOOPS MUSIC",
  labelImg: "https://image-cdn.nct.vn/provider/2022/06/01/3/d/d/9/1654052613310.jpg",
},

    // Thêm các bài khác theo cấu trúc tương tự, nhớ mỗi bài có id
  ];

  const songsVN = [
   {
  id: "song-8",
  title: "ĐOẠN KỊCH CÂM",
  artist: 'ANH TRAI "SAY HI"',
  album: "Say Hi Collection",
  duration: "3:45",
  img: "public/topic/1.jpg",
  audio: "/music/ĐOẠN KỊCH CÂM.mp3",
  label: "UNIVERSAL MUSIC GROUP",
  labelImg: "https://image-cdn.nct.vn/provider/2020/02/06/7/c/c/b/1580970856708.jpg",
},
{
  id: "vn-2",
  title: "Bỏ Mặc Anh",
  artist: "Tez, Mason Nguyen",
  album: "Bỏ Mặc Anh (Single)",
  duration: "3:28",
  img: "https://image-cdn.nct.vn/song/2025/11/27/k/9/c/z/1764216695483_300.jpg",
  audio: "/music/Bỏ Mặc Anh.mp3",
  label: "UNIVERSAL MUSIC GROUP",
  labelImg: "https://image-cdn.nct.vn/provider/2020/02/06/7/c/c/b/1580970856708.jpg",
},
{
  id: "vn-3",
  title: "Qua Vài Câu Chuyện Tình",
  artist: "GiGi Hương Giang",
  album: "Qua Vài Câu Chuyện Tình (Single)",
  duration: "4:06",
  img: "https://image-cdn.nct.vn/song/2025/10/13/y/F/l/W/1760365417721_300.jpg",
  audio: "/music/Qua Vài Câu Chuyện Tình.mp3",
  label: "UNIVERSAL MUSIC GROUP",
  labelImg: "https://image-cdn.nct.vn/provider/2020/02/06/7/c/c/b/1580970856708.jpg",
},
{
  id: "vn-4",
  title: "EMGAI",
  artist: "DI RUMINH",
  album: "EMGAI (Single)",
  duration: "3:12",
  img: "https://image-cdn.nct.vn/song/2025/11/20/4/I/s/S/1763637807057_300.jpg",
  audio: "/music/EMGAI.mp3",
  label: "The Orchard",
  labelImg: "https://image-cdn.nct.vn/provider/2025/05/30/4/e/4/6/1748601994193.jpg",
},
{
  id: "song-9",
  title: "Anh Sẽ Quên Được Em",
  artist: "QuocKiet, Hayho",
  album: "Anh Sẽ Quên Được Em (Single)",
  duration: "4:02",
  img: "https://image-cdn.nct.vn/song/2025/11/25/1/b/e/5/1764049685571_300.jpg",
  audio: "/music/Anh Sẽ Quên Được Em.mp3",
  label: "LOOPS MUSIC",
  labelImg: "https://image-cdn.nct.vn/provider/2022/06/01/3/d/d/9/1654052613310.jpg",
},
{
  id: "vn-6",
  title: "BÍ MẬT NHỎ",
  artist: 'ANH TRAI "SAY HI", Mason Nguyen',
  album: "Say Hi Collection",
  duration: "3:34",
  img: "https://image-cdn.nct.vn/song/2025/12/01/4/9/s/9/1764562044103_300.jpg",
  audio: "/music/BÍ MẬT NHỎ.mp3",
  label: "UNIVERSAL MUSIC GROUP",
  labelImg: "https://image-cdn.nct.vn/provider/2020/02/06/7/c/c/b/1580970856708.jpg",
},
{
  id: "vn-7",
  title: "ĐƯỜNG CHÂN TRỜI",
  artist: 'ANH TRAI "SAY HI", buitruonglinh',
  album: "Say Hi Collection",
  duration: "3:58",
  img: "https://image-cdn.nct.vn/song/2025/12/01/4/9/s/9/1764562044103_300.jpg",
  audio: "/music/ĐƯỜNG CHÂN TRỜI.mp3",
  label: "UNIVERSAL MUSIC GROUP",
  labelImg: "https://image-cdn.nct.vn/provider/2020/02/06/7/c/c/b/1580970856708.jpg",
},
{
  id: "vn-8",
  title: "VẠN LÝ ĐỘC HÀNH",
  artist: 'ANH TRAI "SAY HI", Karik',
  album: "Say Hi Collection",
  duration: "4:20",
  img: "https://image-cdn.nct.vn/song/2025/12/01/4/9/s/9/1764562044103_300.jpg",
  audio: "/music/VẠN LÝ ĐỘC HÀNH.mp3",
  label: "UNIVERSAL MUSIC GROUP",
  labelImg: "https://image-cdn.nct.vn/provider/2020/02/06/7/c/c/b/1580970856708.jpg",
},
{
  id: "vn-9",
  title: "ĐA NGHI",
  artist: 'ANH TRAI "SAY HI"',
  album: "Say Hi Collection",
  duration: "3:41",
  img: "https://image-cdn.nct.vn/song/2025/10/13/G/i/t/4/1760329180626_300.jpg",
  audio: "/music/DA-NGHI.mp3",
  label: "UNIVERSAL MUSIC GROUP",
  labelImg: "https://image-cdn.nct.vn/provider/2020/02/06/7/c/c/b/1580970856708.jpg",
},

  ];

  const songsINT = [
  {
    id: "intl-1",
    img: "https://image-cdn.nct.vn/song/2018/06/20/7/1/5/3/1529487573799_300.jpg",
    title: "That Girl",
    artist: "Olly Murs",
    album: "24 Hrs",
    duration: "3:05",
    label: "SONY MUSIC",
    labelImg: "https://image-cdn.nct.vn/provider/2020/02/13/8/d/9/4/1581563932392.jpg",
    audio: "/music/That Girl.mp3"
  },
  {
    id: "song-6",
    img: "https://image-cdn.nct.vn/song/2017/08/07/c/b/1/e/1502075660969_300.jpg",
    title: "Havana",
    artist: "Camila Cabello, Young Thug",
    album: "Camila",
    duration: "3:36",
    audio: "/music/Havana.mp3",
    label: "SONY MUSIC",
    labelImg: "https://image-cdn.nct.vn/provider/2020/02/13/8/d/9/4/1581563932392.jpg"
  },
  {
    id: "intl-3",
    img: "https://image-cdn.nct.vn/song/2018/01/25/5/2/d/e/1516891769034_300.jpg",
    title: "Shape Of You",
    artist: "Ed Sheeran",
    album: "÷ (Divide)",
    duration: "3:54",
    audio: "/music/Shape Of You.mp3",
    label: "SONY MUSIC",
    labelImg: "https://image-cdn.nct.vn/provider/2020/02/13/8/d/9/4/1581563932392.jpg"
  },
  {
    id: "song-7",
    img: "https://image-cdn.nct.vn/song/2024/10/18/1/8/a/e/1729211353297_300.jpg",
    title: "APT.",
    artist: "ROSÉ, Bruno Mars",
    album: "APT. (Single)",
    duration: "2:49",
    audio: "/music/APT..mp3",
    label: "WARNER RECORDED MUSIC",
    labelImg: "https://image-cdn.nct.vn/provider/2020/11/12/e/2/a/c/1605147272711.jpg"
  },
  {
    id: "intl-5",
    img: "https://image-cdn.nct.vn/song/2020/06/05/4/f/d/2/1591338842259_300.jpg",
    title: "Still With You",
    artist: "Jung Kook (BTS)",
    album: "Still With You (Single)",
    duration: "3:59",
    audio: "/music/Still With You.mp3",
    label: "INGROOVES MUSIC GROUP",
    labelImg: "https://image-cdn.nct.vn/provider/2023/07/06/4/f/c/4/1688614662954.jpg"
  },
  {
    id: "intl-6",
    img: "https://image-cdn.nct.vn/song/2024/10/26/3/0/d/3/1729941033115_300.jpg",
    title: "Trang Giấy Cuối Cùng / 最后一页",
    artist: "EN",
    album: "Trang Giấy Cuối Cùng",
    duration: "4:12",
    audio: "/music/Trang Giấy Cuối Cùng _ 最后一页.mp3",
    label: "SONY MUSIC",
    labelImg: "https://image-cdn.nct.vn/provider/2020/02/13/8/d/9/4/1581563932392.jpg"
  },
  {
    id: "intl-7",
    img: "https://image-cdn.nct.vn/song/2024/03/15/4/c/b/d/1710485895535_300.jpg",
    title: "You Are Everywhere・2024",
    artist: "Young Captain",
    album: "You Are Everywhere",
    duration: "3:41",
    audio: "/music/You Are Everywhere・2024.mp3",
    label: "SONY MUSIC",
    labelImg: "https://image-cdn.nct.vn/provider/2020/02/13/8/d/9/4/1581563932392.jpg"
  },
  {
    id: "intl-8",
    img: "https://image-cdn.nct.vn/song/2017/09/12/b/f/0/6/1505189629603_300.jpg",
    title: "Fly Away",
    artist: "TheFatRat, Anjulie",
    album: "Fly Away (Single)",
    duration: "3:14",
    audio: "/music/Fly Away.mp3",
    label: "SONY MUSIC",
    labelImg: "https://image-cdn.nct.vn/provider/2020/02/13/8/d/9/4/1581563932392.jpg"
  },
  {
    id: "intl-9",
    img: "https://image-cdn.nct.vn/song/2022/12/06/8/b/9/3/1670322228331_300.jpg",
    title: "50 Feet",
    artist: "SOMO",
    album: "SOMO",
    duration: "3:08",
    audio: "/music/50 Feet.mp3",
    label: "SONY MUSIC",
    labelImg: "https://image-cdn.nct.vn/provider/2020/02/13/8/d/9/4/1581563932392.jpg"
  }

  ];

  const getSongs = () => {
    if (tab === "vn") return songsVN;
    if (tab === "intl") return songsINT;
    return songsAll;
  };

  const songs = getSongs();

  // khi click bài hát
  const playSong = (index) => {
    setSongList(songs);
    setCurrentIndex(index);
  };

  const currentSong = currentIndex !== null ? songList[currentIndex] : null;

  const prevSong = () => {
    if (currentIndex === null) return;
    setCurrentIndex((prev) => (prev === 0 ? songList.length - 1 : prev - 1));
  };

  const nextSong = () => {
    if (currentIndex === null) return;
    setCurrentIndex((prev) => (prev === songList.length - 1 ? 0 : prev + 1));
  };

  const handleAddToLibrary = (song) => {
    console.log("Added to library:", song);
  };

  return (
    <div className="nr-container1">
      <h2 className="nr-title1">Mới phát hành</h2>

      <div className="nr-tabs1">
        <button className={tab === "all" ? "active" : ""} onClick={() => setTab("all")}>TẤT CẢ</button>
        <button className={tab === "vn" ? "active" : ""} onClick={() => setTab("vn")}>VIỆT NAM</button>
        <button className={tab === "intl" ? "active" : ""} onClick={() => setTab("intl")}>QUỐC TẾ</button>
      </div>

      <div className="nr-grid">
        {songs.map((song, index) => (
          <div className="nr-item" key={song.id}>
            <img 
              src={song.img} 
              className="nr-img" 
              onClick={() => playSong(index)} 
            />
            <div className="nr-info1">
              <p className="nr-name1" onClick={() => playSong(index)}>{song.title}</p>
              <p className="nr-artist1" onClick={() => playSong(index)}>{song.artist}</p>
               {song.labelImg && (
              <div className="nr-label">
            <img src={song.labelImg} className="nr-label-img1" alt={song.label} />
            <span className="nr-label-text1">{song.label}</span>
          </div>
        )}
            </div>
          </div>
        ))}
      </div>

      {/* MiniPlayer */}
      {currentSong && (
        <MiniPlayer 
          song={currentSong} 
          onPrev={prevSong} 
          onNext={nextSong} 
          onAddToLibrary={handleAddToLibrary} 
        />
      )}
    </div>
  );
}
